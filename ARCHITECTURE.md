...
- **Entry:** `apps/web/vite.config.ts` => `tanstackStart({ srcDirectory: 'src' }) + viteReact() + tailwindcss() + tsConfigPaths()`. Router defined in `src/router.tsx` via `createRouter({ routeTree, context: { queryClient } })` + `setupRouterSsrQueryIntegration`. In the browser, `getQueryClient()` reuses a shared client singleton (one cache for loaders and components); on the server during SSR, `getQueryClient()` instantiates a fresh `QueryClient` per request to guarantee cache isolation across concurrent users. Routes are thin composers under `src/routes/` (`__root.tsx`, `index.tsx`, `_app.tsx` with `_app/` domain subdirectories, `auth.tsx` with `auth/` subdirectories): each holds `validateSearch`/`beforeLoad`/`loader`/`errorComponent` and renders one feature component. `routeTree.gen.ts` is generated.
...
4. Frontend TanStack Query catches HTTP 401 and the shared client performs one bounded refresh. Browser refresh uses the httpOnly cookie; mobile/trusted clients may use an explicit refresh token. A confirmed invalid session clears auth, while timeout/network outcomes remain retryable and cannot restore a logged-out or switched identity.
...
- [ ] **API Client:** Verify `packages/api-client/src/subclients` and the oRPC client are registered. `pnpm generate:feature invoices invoice --access=tenant-shared` wires both transports automatically.
...
- [ ] **Migrations:** Schema changes ship a Drizzle migration (`pnpm --filter api db:generate`); migrations are strictly append-only new numbered files (e.g. `0007_...`) with `pnpm --filter api db:migrate:check` and `node scripts/verify-migration-lineage.js` passing. Never edit `0000_initial.sql` or existing frozen migrations.

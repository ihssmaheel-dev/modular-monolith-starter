...
- **Router:** `src/router.tsx` creates the router with `createRouter({ routeTree, context: { queryClient } })` + `setupRouterSsrQueryIntegration`, reusing the shared `getQueryClient()` singleton. Routes are thin composers under `src/routes/` (`__root.tsx` with html + providers + `Toaster` + globals.css, `_app.tsx` layout session bootstrap + guards with `_app/` domain directories, `auth.tsx` with `auth/` directory): each holds `validateSearch`/`beforeLoad`/`loader`/`errorComponent` and renders one feature component from `src/features/[domain]/components/`.

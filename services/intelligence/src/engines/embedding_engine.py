...
def embed_text(text: str) -> list[float]:
    ...
    Applies PII sanitization prior to generation if enabled.
    """
    processed = sanitize_pii(text) if settings.PII_REDACTION_ENABLED else text
...
def embed_documents(texts: list[str]) -> list[list[float]]:
    ...
    Applies PII sanitization to all input texts prior to embedding.
    """
    processed = [sanitize_pii(t) if settings.PII_REDACTION_ENABLED else t for t in texts]
    model = get_embedding_model()
    embeddings = list(model.embed(processed))

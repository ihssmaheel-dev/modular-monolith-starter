...
from tests.test_envelope_and_limits import assert_canonical_envelope
...
async def test_hmac_missing_headers_rejected(async_client: AsyncClient):
    ...
    data = response.json()
    assert_canonical_envelope(data, 401, "AI_UNAUTHORIZED")
...
async def test_hmac_malformed_timestamp_rejected(async_client: AsyncClient):
    ...
    data = response.json()
    assert_canonical_envelope(data, 400, "VALIDATION_ERROR")

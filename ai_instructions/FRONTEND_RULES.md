...
- Hierarchical route structure: use domain directories (`src/routes/_app/`, `src/routes/auth/`) to avoid flat file sprawl. Root layout routes (`_app.tsx`, `auth.tsx`) wrap their respective folder. Protected layouts retain the leading underscore (`_app/`) as a pathless route so child URLs do not include `/_app/`.

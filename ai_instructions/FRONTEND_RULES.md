...
- Hierarchical route structure: use domain directories (`src/routes/_app/`, `src/routes/_auth-layout/`) to avoid flat file sprawl. Root layout routes (`_app.tsx`, `_auth-layout.tsx`) wrap their respective folder. Pathless layouts retain the leading underscore (`_app/`, `_auth-layout/`) so child URLs do not include layout prefixes (enabling canonical routes like `/login`, `/register`, `/forgot-password`, `/reset-password`).

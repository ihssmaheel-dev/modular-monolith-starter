...
| `COOKIE_DOMAIN`                     | Empty by default for host-only cookies; optional DNS parent domain (for example `example.com`) for a deliberately tested sibling-subdomain deployment                                                   |
| `DATABASE_URL`                      | Local PostgreSQL URL; production requires TLS with `sslmode=require`, `verify-ca`, or `verify-full`                                                                                                     |
| `DB_DIRECT_URL`                     | Optional direct URL for migrations and migration locking; defaults to `DATABASE_URL`                                                                                                                    |
...
| `OTEL_EXPORTER_OTLP_ENDPOINT`   | `http://127.0.0.1:4318/v1/traces`; trace collector                   |
